...
const { DATE: ANSWER_DATE } = require("../constants/answer-types");
const { DATE: METADATA_DATE } = require("../constants/metadata-types");
...
const getPreviousAnswersForValidation = id =>
  db("Answers")
    ...
    .then(({ answerType, ...rest }) =>
      getPreviousAnswers({ answerTypes: [answerType], ...rest })
    );
